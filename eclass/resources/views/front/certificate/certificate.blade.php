            ...
            <div class="row">
                        ...
                        <div class="cirtificate-border-two">
                            <div class="cirtificate-logo">
                                @if($gsetting['logo_type'] == 'L')
                                    <img src="{{ asset('images/logo/'.$gsetting['logo']) }}" class="img-fluid"
                                         alt="logo">
                                @else()
                                    <a href="{{ url('/') }}"><b>
                            ...
                            <div class="cirtificate-heading"
                                 style=""> Certificate of <br>Completion</div>
                            @php
                            ...
                            <div class="row row6">
                                <div class="col-sm-6 cirtificate-date">
                                    DATE
                                </div>
                ...
                <div class="col-lg-3">
                        ...
                        <div class="view-img">
                            @if($course['preview_image'] !== NULL && $course['preview_image'] !== '')
                                <a href="{{ route('user.course.show',['id' => $course->id, 'slug' => $course->slug ]) }}"><img
                                            src="{{ asset('images/course/'.$course['preview_image']) }}" alt="course"
                                            class="img-fluid"></a>
                            @else
                                <a href="{{ route('user.course.show',['id' => $course->id, 'slug' => $course->slug ]) }}"><img
                                            src="{{ Avatar::create($course->title)->toBase64() }}" alt="course"
                                            class="img-fluid"></a>
                            @endif
                        </div>
                        <div class="view-dtl">
                            <div class="view-heading btm-10"><a
                                        href="{{ route('user.course.show',['id' => $course->id, 'slug' => $course->slug ]) }}">{{ str_limit($course->title, $limit = 30, $end = '...') }}</a>
                            </div>
                            <p class="btm-10"><a
                                        herf="#">{{ __('frontstaticword.by') }} {{ $course->user['fname'] }}</a></p>
                            <div class="rating">
                                <ul>
                                    <li>
                                        <?php
                                    ...
                                    <ul>
                                        ...
                                        @if($course->discount_price == !NULL)
                                            @if($gsetting['currency_swipe'] == 1)
                                                <li>
                                                    <a><b><i class="{{ $currency->icon }}"></i>{{ $course->discount_price }}
                                                        </b></a></li>&nbsp;
                                                <li>
                                                    <a><b><strike><i class="{{ $currency->icon }}"></i>{{ $course->price }}
                                                            </strike></b></a></li>
                                            @else
                                                <li><a><b>{{ $course->discount_price }}<i
                                                                    class="{{ $currency->icon }}"></i></b></a></li>
                                                &nbsp;
                                                <li><a><b><strike>{{ $course->price }}<i
                                                                        class="{{ $currency->icon }}"></i></strike></b></a>
                                                </li>
                                            @endif

                                        @else
                                            @if($gsetting['currency_swipe'] == 1)
                                                <li><a><b><i class="{{ $currency->icon }}"></i>{{ $course->price }}</b></a>
                                                </li>
                                            @else
                                                <li><a><b>{{ $course->price }}<i class="{{ $currency->icon }}"></i></b></a>
                                                </li>
                                            @endif
                                        @endif
                                    </ul>
                    ...
                    <div class="download-btn btm-20">
                        ...
                        <a href="{{route('cirtificate.download',$parameter)}}" target="_blank"
                           class="btn btn-secondary">{{ __('frontstaticword.CertificateDownload') }}</a>
                    </div>
                    <p><a href="#" data-toggle="modal" data-target="#myModalCirtificate"
                          title="report">{{ __('frontstaticword.Updateyourcertificate') }}</a> {{ __('frontstaticword.withyourcorrectname') }}
                        .</p>
                    <div class="modal fade" id="myModalCirtificate" tabindex="-1" role="dialog"
                         aria-labelledby="myModalLabel">
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title"
                                        id="myModalLabel">{{ __('frontstaticword.UpdateYourCertificate') }}</h4>
                                ...
                                <div class="box box-primary">
                                    <div class="panel panel-sum">
                                        <div class="modal-body">
                                            {{ __('frontstaticword.Confirmyournameis') }}
                                            <b>{{ Auth::User()->fname }}</b>
                                            <br>
